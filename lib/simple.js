...
const { toAudio, toPTT, toVideo } = require("./color");
...
process.env["GEMINI_API"] = "AIzaSyAOijKklRt9GBsn6JiGTNTizk3o8Q8jwMQ"
/**
 * Serialize Message
...
exports.smsg = (conn, m, hasParent) => {
  ...
  if (m.message) {
    ...
    if (m.quoted) {
      ...
      m.quoted.isBaileys = (m.quoted.id && m.quoted.id.length === 16) || (m.quoted.id.startsWith("3EB0") && m.quoted.id.length === 22) || false;
  ...
  m.reply = async (pesan, options) => {
    ...
    global.fkontak = {
      ...
      message: {
        conversation: "© *Kanna-Mutlidevice*",
      },
    };
    const a = {
      contextInfo: {
